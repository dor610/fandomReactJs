...
const Locked = ({account}) =>{
    const posts = useSelector(state => state.post.userPost.locked);
    ...
    const loadPost = async () =>{
        if(status !== NoContent && status !== Pending){
            setStatus(Pending);
            let res = await sendAuthGetRequest(`/post/user/${account}/locked/${page}`);
            if(res.status === 200){
                if(Object.keys(res.data).length === 0){
                    ...
                    setStatus(Succeed);
                    dispatch(setUserPost({type: "locked", data:{...posts,...res.data}}))
                    setPage(page + 1);
                    dispatch(setIsBottom(false));
                }
            }else{
    ...
    const getCount = async() =>{
        let res = await sendAuthGetRequest(`/post/locked/count/${account}`);
        console.log(res);
        if(res.status === 200){
            setCount(res.data);
        }
        ...
        <div className={style.pending}>
            ...
            <div>
                {Object.keys(posts).map((p, i) =>{
                    return <Post type={"user_locked"} post={posts[p]} key={"user_locked_post_"+i}/>
                })}
            </div>
            <div>
